...
export default function LibraryPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="mb-6 text-2xl font-bold">Sign Language Library</h1>
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-md border border-gray-200 bg-white py-2 pl-10 pr-4 text-sm focus:border-purple-500 focus:outline-none focus:ring-1 focus:ring-purple-500"
          />
        </div>
        ...
        {libraryItems.map((item) => (
          <Card key={item.id} className="overflow-hidden">
            ...
            <CardContent className="p-4">
              <div className="mb-2 inline-block rounded-full bg-purple-100 px-2.5 py-0.5 text-xs font-medium text-purple-800">
                {item.category}
              </div>
            ...
            <CardFooter className="flex items-center justify-between border-t p-4">
              <Link href={`/library/${item.id}`}>
                <Button variant="link" className="h-8 p-0 text-purple-600">
                  View Details
                </Button>
              </Link>
